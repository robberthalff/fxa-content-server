...
define([
  ...
  registerSuite({
    ...
    'device panel works with the device refresh event': function () {
      var self = this;
      return FunctionalHelpers.openPage(this, SIGNIN_URL_DEVICE_LIST, '#fxa-signin-header')
        .then(function () {
          return FunctionalHelpers.fillOutSignIn(self, email, FIRST_PASSWORD);
        ...
        // create a fake "device refresh" event from Firefox
        .execute(function () {
          var event = new CustomEvent('WebChannelMessageToContent', {
            detail: {
              id: 'account_updates',
              message: {
                command: 'fxaccounts:device_refresh'
              }
            }
          });
        ...
        // the device should now appear in the list
        .findByCssSelector('.device-name')
          .getVisibleText()
          .then(function (val) {
            assert.equal(val, TEST_DEVICE_NAME, 'device name is correct');
